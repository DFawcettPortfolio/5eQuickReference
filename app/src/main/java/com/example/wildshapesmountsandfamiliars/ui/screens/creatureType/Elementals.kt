...
)
{
    ...
    var elementalSpellEffectOrSummon by remember { mutableStateOf<List<StatBlock>>(emptyList()) }

    LaunchedEffect(Unit) {
        val allCreatures = loadCreatures(context) // ← Your JSON loader
        elementals = allCreatures.filter {
            it.sortingTag?.lowercase()?.split(",")?.map { tag -> tag.trim() }?.contains("elemental") == true
        }
        elementalSpellEffectOrSummon = allCreatures.filter {
            it.sortingTag?.lowercase()?.split(",")?.map { tag -> tag.trim() }?.contains("spell effect or summon") == true
                    &&
                    it.type?.lowercase()?.contains("elemental") == true
        }
    }

    CreatureTypeScreen(
        title = "Elementals",
        statBlocks = elementals,
        spellEffectOrSummons = elementalSpellEffectOrSummon, // or filter if needed
