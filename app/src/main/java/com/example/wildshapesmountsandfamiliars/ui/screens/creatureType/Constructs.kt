...
)
{
    ...
    var constructSpellEffectOrSummon by remember { mutableStateOf<List<StatBlock>>(emptyList()) }

    LaunchedEffect(Unit) {
        val allCreatures = loadCreatures(context) // ← Your JSON loader
        constructs = allCreatures.filter {
            it.sortingTag?.lowercase()?.split(",")?.map { tag -> tag.trim() }?.contains("construct") == true
        }
        constructSpellEffectOrSummon = allCreatures.filter {
            it.sortingTag?.lowercase()?.split(",")?.map { tag -> tag.trim() }?.contains("spell effect or summon") == true
                    &&
                    it.type?.lowercase()?.contains("construct") == true
        }
    }

    CreatureTypeScreen(
        title = "Constructs",
        statBlocks = constructs,
        spellEffectOrSummons = constructSpellEffectOrSummon, // or filter if needed
