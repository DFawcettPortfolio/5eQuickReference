...
)
{
    ...
    var humanoidSpellEffectOrSummon by remember { mutableStateOf<List<StatBlock>>(emptyList()) }

    LaunchedEffect(Unit) {
        val allCreatures = loadCreatures(context) // ← Your JSON loader
        humanoids = allCreatures.filter {
            it.sortingTag?.lowercase()?.split(",")?.map { tag -> tag.trim() }?.contains("humanoid") == true
        }
        humanoidSpellEffectOrSummon = allCreatures.filter {
            it.sortingTag?.lowercase()?.split(",")?.map { tag -> tag.trim() }?.contains("spell effect or summon") == true
                    &&
                    it.type?.lowercase()?.contains("humanoid") == true
        }
    }

    CreatureTypeScreen(
        title = "Humanoids",
        statBlocks = humanoids,
        spellEffectOrSummons = humanoidSpellEffectOrSummon, // or filter if needed
