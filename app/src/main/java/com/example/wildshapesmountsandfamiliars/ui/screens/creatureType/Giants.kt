...
)
{
    ...
    var giantSpellEffectOrSummon by remember { mutableStateOf<List<StatBlock>>(emptyList()) }

    LaunchedEffect(Unit) {
        val allCreatures = loadCreatures(context) // ← Your JSON loader
        giants = allCreatures.filter {
            it.sortingTag?.lowercase()?.split(",")?.map { tag -> tag.trim() }?.contains("giant") == true
        }
        giantSpellEffectOrSummon = allCreatures.filter {
            it.sortingTag?.lowercase()?.split(",")?.map { tag -> tag.trim() }?.contains("spell effect or summon") == true
                    &&
                    it.type?.lowercase()?.contains("giant") == true
        }
    }
    ...
    CreatureTypeScreen(
        title = "Giants",
        statBlocks = giants,
        spellEffectOrSummons = giantSpellEffectOrSummon, // or filter if needed
