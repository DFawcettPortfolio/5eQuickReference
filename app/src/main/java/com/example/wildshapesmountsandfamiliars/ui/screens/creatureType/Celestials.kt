...
)
{
    ...
    var celestialSpellEffectOrSummon by remember { mutableStateOf<List<StatBlock>>(emptyList()) }

    LaunchedEffect(Unit) {
        val allCreatures = loadCreatures(context) // ← Your JSON loader
        celestials = allCreatures.filter {
            it.sortingTag?.lowercase()?.split(",")?.map { tag -> tag.trim() }?.contains("celestial") == true
        }
        celestialSpellEffectOrSummon = allCreatures.filter {
            it.sortingTag?.lowercase()?.split(",")?.map { tag -> tag.trim() }?.contains("spell effect or summon") == true
                    &&
                    it.type?.lowercase()?.contains("celestial") == true
        }
    }

    CreatureTypeScreen(
        title = "Celestials",
        statBlocks = celestials,
        spellEffectOrSummons = celestialSpellEffectOrSummon, // or filter if needed
