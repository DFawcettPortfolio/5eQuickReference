...
)
{
    ...
    var oozeSpellEffectOrSummon by remember { mutableStateOf<List<StatBlock>>(emptyList()) }

    LaunchedEffect(Unit) {
        val allCreatures = loadCreatures(context) // ← Your JSON loader
        oozes = allCreatures.filter {
            it.sortingTag?.lowercase()?.split(",")?.map { tag -> tag.trim() }?.contains("ooze") == true
        }
        oozeSpellEffectOrSummon = allCreatures.filter {
            it.sortingTag?.lowercase()?.split(",")?.map { tag -> tag.trim() }?.contains("spell effect or summon") == true
                    &&
                    it.type?.lowercase()?.contains("ooze") == true
        }
    }
    ...
    CreatureTypeScreen(
        title = "Oozes",
        statBlocks = oozes,
        spellEffectOrSummons = oozeSpellEffectOrSummon, // or filter if needed
